...
const char *eid_string = "5913715d278927cc947106e874066d926a283630"; // "INSERT_YOUR_ADVERTISEMENT_KEY_HERE"
...
static void ble_start_advertising(uint8_t *adv_raw_data, size_t adv_raw_data_len)
{
    ...
    rc = ble_gap_adv_set_data(adv_raw_data, adv_raw_data_len);
    if (rc != 0) {
    ...
    // Start advertising
    rc = ble_gap_adv_start(BLE_OWN_ADDR_PUBLIC, NULL, BLE_HS_FOREVER, &adv_params, ble_advertise_cb, NULL);
    if (rc != 0) {
...
static void on_sync(void)
{
    ...
    // Print confirmation that advertising has started
    ESP_LOGI(TAG, "=== BLE ADVERTISING STARTED ===");
    ESP_LOGI(TAG, "Device is now discoverable as ESP32-C3-BLE");
    
    // Print adv raw data as hex bytes
